...
namespace CourseWork.Model
{
    public class Star : ModelItem //IDbParseable
    {
        ...
        protected override void ConfigureDataFields() // Агрегирует значения полей таблицы в состояние объекта
        {
            ...
            AddFieldConfig(item =>
            {
                if (item[0] == null || DBNull.Value.Equals(item[0]))
                    Date = null;
                else
                    Date = Convert.ToInt32(item[0]);
            }, "date");
            ...
            AddFieldConfig(item =>
            {
                if (item[0] == null)
                    ...
                    Mass = Convert.ToDouble(item[0].ToString().Replace('.', ','));
            }, "mass");
        ...
        protected override void ConfigureFieldGetters() // Разворачивает состояние объекта в поля таблицы
        {
            ...
            FieldsGetters.Add("date", () => Date == null ? null : Date.ToString());
            ...
            FieldsGetters.Add("mass", () => Mass == null ? null : Mass.ToString().Replace(',', '.'));
